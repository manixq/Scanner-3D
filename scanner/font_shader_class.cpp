...
{
    bool result;
    result = Initialize_shader(device, hwnd, L"shaders/vertex/font.vs", L"shaders/pixel/font.ps");
    if (!result)
        return false;
    return true;
}
...
bool FontShaderClass::Render(ID3D11DeviceContext* device_context, int index_count, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX projection, ID3D11ShaderResourceView* texture, D3DXVECTOR4 pixel_color)
{
    bool result;
    result = Set_shader_parameters(device_context, world, view, projection, texture, pixel_color);
    if (!result)
        return false;
...
bool FontShaderClass::Initialize_shader(ID3D11Device* device, HWND hwnd, WCHAR* vs_filename, WCHAR* ps_filename)
{
    HRESULT result;
    ...
    result = D3DX11CompileFromFile(vs_filename, nullptr, nullptr, "Font_vertex_shader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, nullptr, &vertex_shader_buffer, &error_message, nullptr);
    if (FAILED(result))
    {
        if (error_message)
    ...
    result = D3DX11CompileFromFile(ps_filename, nullptr, nullptr, "Font_pixel_shader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, nullptr, &pixel_shader_buffer, &error_message, nullptr);
    if (FAILED(result))
    {
        if (error_message)
    ...
    pixel_buffer_desc.ByteWidth = sizeof(PIXEL_BUFFER_TYPE);
    ...
    result = device->CreateBuffer(&pixel_buffer_desc, nullptr, &pixel_buffer_);
    if (FAILED(result))
        return false;
...
void FontShaderClass::Shutdown_shader()
{
    if (sample_state_)
    {
...
void FontShaderClass::Output_shader_error_message(ID3D10Blob* error_message, HWND hwnd, WCHAR* shader_filename)
{
    char* compile_errors;
...
bool FontShaderClass::Set_shader_parameters(ID3D11DeviceContext* device_context, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX projection, ID3D11ShaderResourceView* texture, D3DXVECTOR4 pixel_color)
{
    HRESULT result;
    D3D11_MAPPED_SUBRESOURCE mapped_resource;
    MATRIX_BUFFER_TYPE* data_ptr;
    PIXEL_BUFFER_TYPE* data_ptr2;
    ...
    result = device_context->Map(pixel_buffer_, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_resource);
    if (FAILED(result))
        return false;

    data_ptr2 = (PIXEL_BUFFER_TYPE*)mapped_resource.pData;
    data_ptr2->pixel_color = pixel_color;
...
void FontShaderClass::Render_shader(ID3D11DeviceContext* device_context, int index_count)
{
